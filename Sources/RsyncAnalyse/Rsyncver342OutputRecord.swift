...
// MARK: - Rsync 3.4.2 Itemized Output Parser

/// Parser for rsync 3.4.2 itemized output format (11-character format).
/// Format: YXcstpoguax where Y=update type, X=file type, c-x=attributes (9 positions).
/// Total format length: 11 characters + space + path.
public struct Rsyncver342OutputRecord {
    ...
    /// Parse rsync 3.4.2 output record with automatic format detection
    ...
    /// Parse strict 11-character rsync 3.4.2 format: ".f..t...... file.txt"
    private static func parseStrictFormat(_ record: String) -> Rsyncver342OutputRecord? {
        ...
        return Rsyncver342OutputRecord(
            path: path,
